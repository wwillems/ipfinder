function showIP(latitude, longitude) {
	$('#map').gmap3(
	  { action: 'addMarker',
...
function getLatLong(urlObj, options) {
	...
	if (qrUrl) {
		...
		var $content = $page.children(":jqmData(role=content)");
	  	...
	  	$.getJSON("http://freegeoip.net/json/" + qrUrl + "?callback=?",
  		function(data){
			$content.find("#country").text(data.country_name + " (" + data.country_code + ")");
  			if (data.region_name != '' ) {
  				$content.find("#region").text(data.region_name + " (" + data.region_code + ")");
  			} else {
				$content.find("#region").text("n/a");
			}
			if (data.region_name != '' ) {
  			   $content.find("#city").text(data.city);
			} else {
			  $content.find("#city").text("n/a");
			}
			if (data.region_name != '' ) {
  			    $content.find("#zipcode").text(data.zipcode);
			} else {
			    $content.find("#zipcode").text("n/a");
			}
  			$content.find("#latlng").text(data.latitude + ", " + data.longitude);
  			
  			$(function() {
  				showIP(data.latitude, data.longitude);
  			});
  		});
...
$(document).bind( "pagebeforechange", function(e, data) {
	...
	if (typeof data.toPage === "string") {
		...
		else if (u.hash.search(googlemap) !== -1) {
			// Display google map for the selected URL.
			getLatLong(u, data.options);
			e.preventDefault();
		}
...
function showUrlList(urlObj, options) {
  ...
  for (var i=0; i<myUrls.length; i++) {
		markup = markup + "<li><a href='#googlemap?url=" + myUrls[i] + "'>" + getHostname(myUrls[i]) + "</a>" + "<a href='#delurl?url=" + myUrls[i] + "' data-rel='dialog'>Delete</a></li>";
  }	
	markup = markup + "</ul>";
...
function getMyUrls() {
	...
	if (storedUrls) {
		...
		// Initialize defaults
		myUrls = [encodeURIComponent("www.google.com")];
